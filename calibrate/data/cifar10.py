...
class IndexWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        data = self.dataset[index] # Returns (input, label)
        return data + (index,)     # Returns (input, label, index)
...
def get_train_valid_loader(batch_size,
                           ...
                           pin_memory=False,
                           get_val_temp=0,
                           data_dir="./data", return_index=False):
    """
    ...
    if return_index:
        train_dataset = IndexWrapper(train_dataset)
    else:
        pass
