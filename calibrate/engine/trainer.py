...
class Trainer:
    ...
    def build_solver(self) -> None:
        ...
        self.optimizer = instantiate(
            self.cfg.optim.object, chain(self.model.parameters(), self.loss_func.parameters())
        )
    ...
    def train_epoch(self, epoch: int):
        ...
        for i, (inputs, labels) in enumerate(self.train_loader):
            ...
            if self.cfg.loss.get('requires_features', False):
                outputs, features = self.model(inputs,return_features=True)
                loss = self.loss_func(outputs, labels,features)
            else:                
